    ...
    public class JwtTokenService : IJwtTokenService
    {
        ...
        public Result<string> GenerateAccessToken(IEnumerable<Claim> claims)
        {
            try
            {
                ...
                JwtSecurityToken tokenOptions = new(_jwtOption.Issuer, _jwtOption.Audience, claims, expires: DateTime.UtcNow.AddMinutes(_jwtOption.ExpiryMinutes),
        ...
        public async Task<Result<IDictionary<string, RedisValue>>> GetSessionAsync(int userId, string deviceId)
        {
            try
            {
                ...
                return Result.Success<IDictionary<string, RedisValue>>(sessionData.ToDictionary(x => x.Name.ToString(), x => x.Value));
            }
            catch (Exception ex)
            {
        ...
        public async Task<Result> SaveSessionAsync(int userId, string deviceId, string accessToken, string refreshToken)
        {
            try
            {
                ...
                {
                    new("accessToken", accessToken), new("refreshToken", refreshToken), new("refreshTokenExpiry", DateTimeOffset.UtcNow.Add(expiry).ToUnixTimeSeconds())
                };
