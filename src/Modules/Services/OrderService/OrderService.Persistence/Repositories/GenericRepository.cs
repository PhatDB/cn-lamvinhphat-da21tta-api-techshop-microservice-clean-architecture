...
namespace OrderService.Persistence.Repositories
{
    public class GenericRepository<T> : IGenericRepository<T> where T : Entity, IAggregateRoot
    {
        ...
        public async Task<Result<T>> GetByIdAsync(int id, CancellationToken cancellationToken = default)
        {
            ...
            return entity is null ? Result.Failure<T>(Error.NotFound("Entity.NotFound", $"{typeof(T).Name} with ID {id} not found.")) : Result.Success(entity);
        }
        ...
        {
            bool exists = await _dbSet.AnyAsync(e => EF.Property<int>(e, "Id") == id, cancellationToken);
        ...
        {
            T? entity = await _dbSet.AsNoTracking().FirstOrDefaultAsync(predicate, cancellationToken);

            return entity is null ? Result.Failure<T>(Error.NotFound("Entity.NotFound", $"{typeof(T).Name} not found with specified condition.")) : Result.Success(entity);
        }
